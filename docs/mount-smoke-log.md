...
本文件归档 `build/mount-smoke.mjs` 的红/绿运行记录。脚本每一步都打印 `PASS`/`FAIL`，
结束时打印唯一一行 `SMOKE GREEN|RED (n/m)` 并以退出码反映结论。

复现方式（需先把包挂进 web profile）：
...
| 步骤 | 断言 |
|------|------|
| boot | `dsh --profile web --no-open --port N` 起得来并打印带 token 的 URL（任何装载期硬失败都会在此暴露为超时或非零退出） |
| page | 带 token 请求 `/` 时服务端以 303 下发鉴权 cookie，跟随后渲染出 shell |
| roster | 启动图里存在 `dsh-ai-coding` 的客户端模块记录，并带有加载器将要请求的 URL |
| bundle | 该 URL 返回 200，且内容是本包的闭包工厂产物（首行 `window.__ModuleLoader__.load(`） |
...
- 绿 4 次 / 红 1 次，满足「≥3 次绿方可宣布通过」。
- 红绿差异只出现在 `roster` 一步：卸载后启动图里没有本包的客户端记录，
  其余三步照常通过 —— 说明该断言确实盯着「本插件是否被装载」，
  而不是任何服务的通用健康检查。
- 复现脚本已入库：`build/mount-smoke.mjs`。
